...
void Application::initVulkan() {
	...
	m_Uniformbuffer.createDescriptorSetLayout(m_logicalDevice, m_CommandPool->GetMaxFramesInFlight());
	...
	// decode first frame so we know dimensions
	m_videoPlayer->decodeNextFrame();

	// cvreate texture at video resolution
	m_textureVk = std::make_unique<TextureVK>();
	m_textureVk->setAllocator(m_logicalDevice.GetAllocator());
	m_textureVk->CreateTextureForVideo(m_Uniformbuffer, m_logicalDevice, *m_CommandPool, 
		m_videoPlayer->getWidth(), m_videoPlayer->getHeight());

	// upload first decoded frame
	m_textureVk->updateTexture(m_videoPlayer->getFrameData(),
		m_videoPlayer->getWidth(), m_videoPlayer->getHeight(),
